...
class SdJwtKb(
    ...
    fun verify(
        ...
        // TODO: make sure we perform all checks in Section 7.1 through 7.3

        // Note: it's possible the jwk method isn't used but that's the only one we support right now.
        val kbKey = EcPublicKey.fromJwk(sdJwt.jwtBody["cnf"]!!.jsonObject["jwk"]!!.jsonObject)
        try {
            JsonWebSignature.verify(JsonPrimitive("$kbHeader.$kbBody.$kbSignature"), kbKey)
        } catch (e: Throwable) {
