...
CredentialDatabase::CredentialDatabase(const uint8_t* encodedDatabase, size_t encodedDatabaseLength) {
    ...
    auto appInfo = topMap->get("app_info")->asMap();
    appName = appInfo->get("name")->asTstr()->value();
    appIcon = appInfo->get("icon")->asBstr()->value();
    continueToWalletMessage = appInfo->get("message")->asTstr()->value();
