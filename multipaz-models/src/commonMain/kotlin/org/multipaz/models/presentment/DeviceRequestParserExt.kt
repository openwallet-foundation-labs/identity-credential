...
/**
 * Generates a list of [PresentmentCredentialSet], one for each [MdocCredential] satisfying the doc request.
 *
 ...
 * @return a [PresentmentData] or `null` if no credentials satisfy the request.
 */
suspend fun DeviceRequestParser.DocRequest.getPresentmentData(
    ...
    for (document in documents) {
        ...
        /*
        matches.add(PresentmentCredentialSetOptionMemberMatch(
    ...
    /*
    return listOf(PresentmentCredentialSet(
        options = listOf(
            PresentmentCredentialSetOption(
                matches = matches
            )
        ),
        dcqlResponsePath = listOf()
    ))
     */
    // TODO
    TODO()
