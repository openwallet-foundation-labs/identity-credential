...
object ASN1 {
    ...
    internal fun decode(derEncoded: ByteArray, offset: Int): Pair<Int, ASN1Object?> {
        ...
        val decodedObject = if (idOctets.cls == ASN1TagClass.UNIVERSAL) {
            when (idOctets.tagNumber) {
                ...
                ASN1Integer.TAG_NUMBER -> ASN1Integer.parse(content)
                ...
                ASN1Set.TAG_NUMBER -> ASN1Set.parse(content)
                    ...
                    if (ASN1StringTag.entries.find { it.tagNumber == idOctets.tagNumber } != null) {
    ...
    fun print(
        ...
        when (obj) {
            ...
            is ASN1Integer -> {
                // TODO: always print as integer
                try {
                    sb.append("INTEGER ${obj.toLong()}\n")
                } catch (e: IllegalStateException) {
                    sb.append("INTEGER ${obj.value.toHex()}\n")
                }
            }
            ...
            is ASN1Time -> {
                sb.append("UTCTime ${obj.value}\n")
            }
            is ASN1String -> {
                val label = when (obj.tagNumber) {
                    ASN1StringTag.UTF8_STRING_TAG_NUMBER.tagNumber -> "UTF8String"
                    ASN1StringTag.NUMERIC_STRING_TAG_NUMBER.tagNumber -> "NumericString"
                    ASN1StringTag.PRINTABLE_STRING_TAG_NUMBER.tagNumber -> "PrintableString"
                    ASN1StringTag.TELETEX_STRING_TAG_NUMBER.tagNumber -> "TeletexString"
                    ASN1StringTag.VIDEOTEX_STRING_TAG_NUMBER.tagNumber -> "VideotexString"
                    ASN1StringTag.IA5_STRING_TAG_NUMBER.tagNumber -> "IA5String"
                    ASN1StringTag.GRAPHIC_STRING_TAG_NUMBER.tagNumber -> "GraphicString"
                    ASN1StringTag.VISIBLE_STRING_TAG_NUMBER.tagNumber -> "VisibleString"
                    ASN1StringTag.GENERAL_STRING_TAG_NUMBER.tagNumber -> "GeneralString"
                    ASN1StringTag.UNIVERSAL_STRING_TAG_NUMBER.tagNumber -> "UniversalString"
                    ASN1StringTag.CHARACTER_STRING_TAG_NUMBER.tagNumber -> "CharacterString"
                    ASN1StringTag.BMP_STRING_TAG_NUMBER.tagNumber -> "BmpString"
            ...
            is ASN1RawObject -> {
                ...
                sb.append("tag=${obj.tagNumber} value=${obj.value.toHex()}")
            }
