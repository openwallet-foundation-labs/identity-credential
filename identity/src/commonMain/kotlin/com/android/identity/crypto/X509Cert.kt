...
class X509Cert(
    ...
    val subjectKeyIdentifier: ByteArray?
        get() {
            val extVal = getExtensionValue(OID.X509_EXTENSION_SUBJECT_KEY_IDENTIFIER.oid)!!
    ...
    val authorityKeyIdentifier: ByteArray?
        get() {
            val extVal = getExtensionValue(OID.X509_EXTENSION_AUTHORITY_KEY_IDENTIFIER.oid)!!
    ...
    val keyUsage: Set<X509KeyUsage>
        get() {
            ...
            val extValue = ASN1.decode(getExtensionValue(OID.X509_EXTENSION_KEY_USAGE.oid)!!)
            return X509KeyUsage.decodeSet(extValue as ASN1BitString)
        }
