...
class CborTests {
    private fun cborToString(item: DataItem) =
        Cbor.toDiagnostics(
            item,
            setOf(DiagnosticOption.EMBEDDED_CBOR, DiagnosticOption.PRETTY_PRINT),
        )

    private fun encodeNumber(value: Long) =
        Cbor.toDiagnostics(
            Cbor.encode(value.toDataItem),
            setOf(DiagnosticOption.EMBEDDED_CBOR, DiagnosticOption.PRETTY_PRINT),
        )

    private fun encodeFloat(value: Float) =
        Cbor.toDiagnostics(
            Cbor.encode(value.toDataItem),
            setOf(DiagnosticOption.EMBEDDED_CBOR, DiagnosticOption.PRETTY_PRINT),
        )
    ...
    //
    private val diagnosticsTestVectors =
        listOf(
            TestVector("0", "00"),
            TestVector("1", "01"),
            TestVector("10", "0a"),
    ...
    private val nonWellformedExamples =
        listOf(
