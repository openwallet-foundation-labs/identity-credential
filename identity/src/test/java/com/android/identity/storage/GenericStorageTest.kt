...
class GenericStorageTest {
    ...
    fun testStorageImplementation() {
        val storageDir = Files.createTempDirectory("ic-test").toFile()
        val storage: StorageEngine = GenericStorageEngine(storageDir)
        storage.deleteAll()
    ...
    fun testPersistence() {
        val storageDir = Files.createTempDirectory("ic-test").toFile()
        var storage: StorageEngine = GenericStorageEngine(storageDir)
        storage.deleteAll()
        Assert.assertEquals(0, storage.enumerate().size.toLong())
        Assert.assertNull(storage["foo"])
        val data = byteArrayOf(1, 2, 3)
        ...
        storage = GenericStorageEngine(storageDir)
        Assert.assertArrayEquals(storage["foo"], data)
    }
}
