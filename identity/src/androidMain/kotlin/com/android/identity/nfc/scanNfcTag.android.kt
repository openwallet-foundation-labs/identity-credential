...
import com.android.identity.identity.generated.resources.Res
import com.android.identity.identity.generated.resources.nfc_tag_reader_icon_error
import com.android.identity.identity.generated.resources.nfc_tag_reader_icon_scan
import com.android.identity.identity.generated.resources.nfc_tag_reader_icon_success
...
private class NfcTagReader<T> {
    ...
    private val nfcReaderModeListener = object : NfcAdapter.ReaderCallback {
        override fun onTagDiscovered(tag: Tag?) {
            ...
            Logger.i(TAG, "onTagDiscovered() in thread ${Thread.currentThread()}")
                ...
                if (tech == IsoDep::class.java.name) {
                    ...
                    runBlocking {
                        try {
                            ...
                            Logger.i(TAG, "Lost tag during interaction, continuing")
    ...
    private val adapter = NfcAdapter.getDefaultAdapter(AndroidInitializer.applicationContext)
    ...
    private val dialogIcon = mutableStateOf(Res.drawable.nfc_tag_reader_icon_scan)
    ...
    suspend fun beginSession(
        ...
        try {
            ...
            dialogIcon.value = Res.drawable.nfc_tag_reader_icon_success
            vibrateSuccess()
            ...
            dialogIcon.value = Res.drawable.nfc_tag_reader_icon_error
            ...
            disableReaderModeJob = CoroutineScope(Dispatchers.IO).launch {
                ...
                Logger.i(TAG, "Disabled reader mode on timeout")
                disableReaderModeJob = null
            }
        }
    }
