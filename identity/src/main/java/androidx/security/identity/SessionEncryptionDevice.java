...
public class SessionEncryptionDevice {

    private static final String TAG = "SessionEncryptionDevice";

    private boolean mEReaderKeyReceived = false;

    private byte[] mEncodedEReaderKeyPub = null;
    private PublicKey mEReaderKeyPub = null;

    private final PrivateKey mEDeviceKeyPrivate;
    private final byte[] mEncodedDeviceEngagement;
    private final DataItem mHandover;

    private SecretKeySpec mSKDevice = null;
    private SecretKeySpec mSKReader = null;
    private int mSKDeviceCounter = 1;
    private int mSKReaderCounter = 1;


    // Calculated upon receipt of first message, in ensureSessionEncryptionKeys().
    //
    ...
    public @Nullable Pair<byte[], OptionalInt> decryptMessageFromReader(
        ...
        try {
            ...
            throw new IllegalArgumentException("Data is not valid CBOR");
        }
        if (dataItems.size() != 1) {
    ...
    private void ensureSessionEncryptionKeys() {
        if (mSKReader != null) {
            return;
        }
