...
public class DataTransportNfc extends DataTransport {
    ...
    private boolean mEndTransceiverThread;
     ...
     *
     * @param responseInterface A way for this implemention to post APDU responses.
     */
                ...
                try {
                    ...
                    while (!mEndTransceiverThread && mIsoDep.isConnected()) {
                        ...
                        do {
                            ...
                            //Log.d(TAG, String.format("chunk length 0x%04x : %s", size, SUtil.toHex(chunk)));
                            ...
                            int bitsPerSec = (int) ((envelopeCommand.length + envelopeResponse.length)
                                                                * 8 / durationSec);
                            ...
                            if (moreChunksComing) {
                                ...
                                Log.d(TAG, "envResponse (more chunks coming) " + Util.toHex(envelopeResponse));
                            } else {
                        ...
                        //Log.d(TAG,"envResponse (have all chunks) " + SUtil.toHex(lastEnvelopeResponse));
                        ...
                        // TODO: we only get 90 00 if the entire response fits in a single
                        //   APDU...
                        byte[] encapsulatedMessage;
                        int status = (lastEnvelopeResponse[erl - 2] & 0xff)*0x100
                                + (lastEnvelopeResponse[erl - 1] & 0xff);
                        if (status == 0x9000) {
                            // Woot, entire response fit in the repsonse APDU
                            //
                            encapsulatedMessage = new byte[erl - 2];
                            System.arraycopy(lastEnvelopeResponse, 0, encapsulatedMessage, 0, erl - 2);
