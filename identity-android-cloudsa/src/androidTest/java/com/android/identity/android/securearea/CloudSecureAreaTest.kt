...
class CloudSecureAreaTest {
    ...
    internal inner class LoopbackCloudSecureArea(
        ...
        onAuthorizeRootOfTrust: (cloudSecureAreaRootCertificate: X509Certificate) -> Boolean
    ) : CloudSecureArea(context, storageEngine, cloudUri, onAuthorizeRootOfTrust) {
        private val server: Server

        init {
            val enclaveBoundKey = Random.Default.nextBytes(32)
            ...
            val attestationKeyCertificates = CertificateChain(listOf(
            ...
            val cloudBindingKeyAttestationCertificates = CertificateChain(listOf(
                Crypto.createX509v3Certificate(
        ...
        override fun communicateLowlevel(requestData: ByteArray): Pair<Int, ByteArray> {
            return server.handleCommand(requestData, "http://localhost:8080/cloudsa-server")
        }

        override fun delayForBruteforceMitigation(duration: Duration) {
    ...
    fun testKeyCreation() {
        ...
        val csa = LoopbackCloudSecureArea(
            ...
            "not-used-by-loopback-server",
            null
        ) { cloudSecureAreaRootCertificate: X509Certificate -> true }
        csa.register()
    ...
    fun testKeySigning() {
        ...
        Assert.assertEquals(setOf<Any>(), keyInfo.userAuthenticationType)
    ...
    @Test
    fun testServerChecksAttestationForApplication() {
        val context = InstrumentationRegistry.getTargetContext()
    ...
    @Test
    fun testServerChecksAttestationForApplicationNegative() {
        val context = InstrumentationRegistry.getTargetContext()
    ...
    @Test
    fun testWrongPassphraseDelay_signing() {
        testWrongPassphraseDelayHelper(useKey = { alias, csa, unlockData ->
            csa.sign(alias, Algorithm.ES256, byteArrayOf(1, 2, 3), unlockData)
        })
    }

    @Test
    fun testWrongPassphraseDelay_keyAgreement() {
    ...
    fun testWrongPassphraseDelayHelper(
        useKey: (alias: String,
                 csa: CloudSecureArea,
        ...
        csa.createKey("testKey1",
            CloudCreateKeySettings.Builder(byteArrayOf())
                ...
                .setPassphraseRequired(true, "1111")
                .build()
        )
        ...
        csa.createKey("testKey2",
            CloudCreateKeySettings.Builder(byteArrayOf())
                .setKeyPurposes(setOf(KeyPurpose.SIGN, KeyPurpose.AGREE_KEY))
                .setPassphraseRequired(true, "1234")
                .build()
        )
        ...
        val correctPassphrase = CloudKeyUnlockData("testKey1")
        correctPassphrase.passphrase = "1111"

        val incorrectPassphrase = CloudKeyUnlockData("testKey1")
        incorrectPassphrase.passphrase = "1112"

        val correctPassphraseTestKey2 = CloudKeyUnlockData("testKey2")
        correctPassphraseTestKey2.passphrase = "1234"
        ...
        // CloudSecureArea.sleepForBruteforceMitigation() method which does just that
        ...
        csa2.createKey("client2_testKey1",
            CloudCreateKeySettings.Builder(byteArrayOf())
                .setKeyPurposes(setOf(KeyPurpose.SIGN, KeyPurpose.AGREE_KEY))
                .setPassphraseRequired(true, "9876")
                .build()
        )
        val correctPassphraseClient2 = CloudKeyUnlockData("client2_testKey1")
    ...
    companion object {
        fun getAttestationChallenge(certChain: CertificateChain): ByteArray {
            val x509cert = certChain.certificates[0].javaX509Certificate
