...
- `identity-cloudsa` and `identity-android-cloudsa` are experimental libraries
   for implementing and communicating with a Cloud-based Secure Area. This is
...
- A light-weight _Secure Area_ abstraction for hardware-backed keystore
  ...
  - A protocol for a Cloud Secure Area is provided along with production quality
    client-side implementation in the `identity-android-cloudsa` library and a reference
    implementation of the server side in the `identity-cloudsa` library with the caveat
