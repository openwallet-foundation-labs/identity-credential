...
class VerifierRequestDialog(private val consentItems : List<String>,
                            val onConsent : (List<String>) -> Unit,
                            val onCancel : () -> Unit) : DialogFragment() {

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return activity?.let { activity ->
            val selectedItems = ArrayList<String>()
            selectedItems.addAll(consentItems)
            selectedItems.add("portrait")

            val builder = AlertDialog.Builder(activity)
            // Set the dialog title
            builder.setTitle(R.string.verifier_request_dialog)
                // Specify the list array, the items to be selected by default (null for none),
                // and the listener through which to receive callbacks when items are selected
                .setMultiChoiceItems(getConsentLabels(consentItems, activity.applicationContext),
                    consentItems.map { true }.toBooleanArray()
                ) { _, which, isChecked ->
                    if (isChecked) {
                        selectedItems.add(consentItems[which])
                    } else {
                        selectedItems.remove(consentItems[which])
                    }
                }
