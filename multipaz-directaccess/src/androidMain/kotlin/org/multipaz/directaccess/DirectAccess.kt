...
object DirectAccess {
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    suspend fun getMaximumCredentialSize(): Long {
            val apdu: ByteArray?
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    suspend fun isDirectAccessSupported(): Boolean{
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    suspend fun allocateDocumentSlot(docType: String): Int {
        val apdu: ByteArray?
        val response: ByteArray?
        ...
        try {
            ...
            check(getAPDUResponseStatus(response) == APDU_RESPONSE_STATUS_OK) //todo:ak:at some point the APDU stops allocating with -1
            Logger.i(TAG, "allocateDocumentSlot success.")
    ...
    suspend fun clearDocumentSlot(
        ...
        try {
            ...
            Logger.i(TAG, "clearDocumentSlot success.")
            return true
        } catch (e: IOException) {
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    suspend fun enumerateAllocatedSlots(): List<Int> {
        val apdu: ByteArray?
        val response: ByteArray?
        try {
            transport.closeConnection()
    ...
    suspend fun createCredential(
        ...
        try {
            ...
            Logger.i(TAG, "createPresentationPackage success.")
    ...
    suspend fun certifyCredential(
        ...
        try {
            ...
            Logger.i(TAG, "certifyCredential success.")
        } catch (e: IOException) {
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    suspend fun getCredentialUsageCount(
        documentSlot: Int,
    ): Int {
        val apdu: ByteArray?
        val response: ByteArray?
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    suspend fun clearCredentialUsageCount(
        documentSlot: Int,
    ) {
        val apdu: ByteArray?
        val response: ByteArray?
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    suspend fun setActiveCredential(
        documentSlot: Int,
        encryptedPresentationData: ByteArray?
    ) {
        try {
            ...
            Logger.i(TAG, "setActiveCredential success.")
