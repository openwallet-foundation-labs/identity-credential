...
suspend fun validateClientAttestation(
    ...
    val attestationBody = validateJwt(
        ...
        publicKey = null,
        checks = mapOf(
            JwtCheck.X5C to "client_attestation",  // where to find CA
...
suspend fun validateClientAssertionJwt(clientAssertionJwt: String, clientId: String) {
    validateJwt(
        jwt = clientAssertionJwt,
        jwtName = "client_assertion",
        publicKey = null,
        checks = mapOf(
            JwtCheck.X5C to "client_assertion",  // where to find CA
            JwtCheck.JTI to clientId,
            JwtCheck.SUB to clientId,
            JwtCheck.ISS to clientId,
