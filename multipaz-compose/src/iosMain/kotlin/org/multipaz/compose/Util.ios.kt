...
actual fun cropRotateScaleImage(
    ...
    val resultUIImage = renderer.imageWithActions { rendererContext ->
        ...
        // Adjust context orientation based on output mode.
        if (!frameData.isLandscape) { // PORTRAIT
            CGContextTranslateCTM(cgContext, 0.0, finalCanvasHeight)
            CGContextScaleCTM(cgContext, 1.0, -1.0)
        } else { // LANDSCAPE
            // Assuming for landscape, the context might already be top-left, Y-down,
            // or the default orientation works correctly without an initial flip.
            // If landscape images are upside down, further adjustments here might be needed.
            CGContextTranslateCTM(cgContext, 0.0, finalCanvasHeight)
            CGContextScaleCTM(cgContext, 1.0, -1.0)
        }
...
/**
 * Helper to convert ImageBitmap to CGImageRef.
 */
@OptIn(ExperimentalForeignApi::class)
fun ImageBitmap.toCGImage(): CGImageRef? {
    val width = this.width
    val height = this.height
    val bytesPerRow = width * 4
    val pixelInts = IntArray(width * height)

    this.readPixels(buffer = pixelInts, startX = 0, startY = 0, width = width, height = height)

    val bitmapBytes = ByteArray(bytesPerRow * height)
    var byteIdx = 0
    for (pixel in pixelInts) {
        ...
        bitmapBytes[byteIdx++] = r
        bitmapBytes[byteIdx++] = g
        bitmapBytes[byteIdx++] = b
        bitmapBytes[byteIdx++] = 255.toByte() // Force opaque alpha.
    }

    return bitmapBytes.usePinned { pinned ->
        val colorSpace = CGColorSpaceCreateDeviceRGB()
        val context = CGBitmapContextCreate(
