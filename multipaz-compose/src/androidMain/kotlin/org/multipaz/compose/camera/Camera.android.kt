...
actual fun Camera(
    ...
    onFrameCaptured: suspend (frame: ImageBitmap) -> ImageBitmap?
) {
    ...
    if (showCameraPreview) {
        if (overlayBitmap?.width != imageBitmap?.width || overlayBitmap?.height != imageBitmap?.height) {
            Log.w(TAG, "Supplied overlay bitmap has the wrong size for the preview " +
        ...
        imageBitmap?.let {
            Image(
                modifier = modifier,
                bitmap = processCameraBitmap(it, overlayBitmap),
        ...
        if (overlayBitmap != null) {
            Log.w(TAG, "Camera received the frame overlay data, but no preview is requested.")
        }
    }
}
