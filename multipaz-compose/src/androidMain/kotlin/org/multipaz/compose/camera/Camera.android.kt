...
actual fun Camera(
    ...
    var currentDisplayRotation by remember { mutableIntStateOf(context.display.rotation) }
    ...
    DisposableEffect(Unit) { // Keyed by Unit to run once and clean up with the composable
        val orientationEventListener = object : OrientationEventListener(context.applicationContext) {
            override fun onOrientationChanged(orientation: Int) {
                val newRotation = context.display.rotation // Get current display rotation
                if (currentDisplayRotation != newRotation) {
                    Logger.d(TAG, "Orientation changed. New display rotation for target: $newRotation")
