...
private object FaceNetInterpreter {
    ...
    fun initialize(imgSize: Int) {
        if (isInitialized) {
            return
        }

        val modelByteBuffer = ModelByteBufferLoader.getModelByteBuffer()
...
@OptIn(InternalResourceApi::class)
actual fun getFaceEmbeddings(image: ImageBitmap, imgSize: Int, embeddingDim: Int): FloatArray? {
    if (!FaceNetInterpreter.isInitialized()) {
        FaceNetInterpreter.initialize(imgSize)
    }
    return FaceNetInterpreter.getEmbeddings(image, embeddingDim)
}
