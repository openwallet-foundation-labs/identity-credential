...
public class SessionEncryptionTest {
    ...
    public void testReaderAgainstVectors() {
        ...
        Pair<byte[], OptionalLong> result = sessionEncryption.decryptMessage(
    ...
    public void testDeviceAgainstVectors() {
        ...
        // Check that decryption works.
        Pair<byte[], OptionalLong> result = sessionEncryptionDevice.decryptMessage(
    ...
    private void testCurve(@Constants.EcCurve int curve) {
        KeyPair eReaderKeyPair = Utility.createEphemeralKeyPair(curve);
        PublicKey eReaderKeyPublic = eReaderKeyPair.getPublic();
        KeyPair eDeviceKeyPair = Utility.createEphemeralKeyPair(curve);
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            byte[] mdocRequest = sessionEncryptionReader.encryptMessage(
                    Util.fromHex(TestVectors.ISO_18013_5_ANNEX_D_DEVICE_REQUEST),
                    OptionalLong.empty());
            Pair<byte[], OptionalLong> result = sessionEncryptionHolder.decryptMessage(
                    mdocRequest);
            Assert.assertFalse(result.second.isPresent());
            Assert.assertArrayEquals(
                    Util.fromHex(TestVectors.ISO_18013_5_ANNEX_D_DEVICE_REQUEST),
