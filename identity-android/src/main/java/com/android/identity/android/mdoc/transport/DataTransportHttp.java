...
public class DataTransportHttp extends DataTransport {
    ...
    DataTransport fromConnectionMethod(@NonNull Context context,
                                       @NonNull ConnectionMethodHttp cm,
        ...
        if (role == DataTransport.ROLE_MDOC_READER) {
            ...
            return new DataTransportHttp(context, role, cm, options);
        }

        // For the mdoc role, this should be an URI pointing to a server on the Internet.
        URI uri = null;
        try {
            uri = new URI(cm.getUri());
        } catch (URISyntaxException e) {
            throw new IllegalArgumentException(e);
        }
        DataTransportHttp transport = new DataTransportHttp(context, role, cm, options);
