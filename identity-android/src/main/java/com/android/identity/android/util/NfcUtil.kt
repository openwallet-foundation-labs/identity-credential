...
object NfcUtil {
        ...
        for (cm in methods) {
            ...
            if (records != null) {
                if (Logger.isDebugEnabled) {
                    Logger.d(
                        TAG,
                        "ConnectionMethod $cm: alternativeCarrierRecord: " +
    ...
    fun findServiceParameterRecordWithName(
        ...
        for (r in m.getRecords()) {
            ...
            if (r.tnf == NdefRecord.TNF_WELL_KNOWN &&
                Arrays.equals(
                    "Tp".toByteArray(), r.type,
                ) && p != null && p.size > snUtf8.size + 2 && p[0].toInt() == 0x10 && p[1].toInt() == snUtf8.size &&
                Arrays.equals(
                    snUtf8,
    ...
    fun fromNdefRecord(
        ...
        if (record.tnf == NdefRecord.TNF_MIME_MEDIA &&
            ...
            return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                Logger.w(
                    TAG,
                    "Ignoring Wifi Aware Carrier Configuration since Wifi Aware " +
