...
actual fun CloudSecureAreaScreen(showToast: (message: String) -> Unit) {
    val showCertificateDialog = remember { mutableStateOf<X509CertChain?>(null) }
    var connectText by remember { mutableStateOf("Click to connect to Cloud Secure Area") }
    var connectColor by remember { mutableStateOf(Color.Red) }
...
@Composable
fun ShowCertificateDialog(attestation: KeyAttestation,
                          onDismissRequest: () -> Unit) {
    var certNumber by rememberSaveable() { mutableStateOf(0) }
    if (certNumber < 0 || certNumber >= attestation.certChain!!.certificates.size) {
        certNumber = 0
    }
    Dialog(onDismissRequest = { onDismissRequest() }) {
        Card(
            ...
            Column(
                ...
                Row() {
                    Text(
                        text = "Certificate ${certNumber + 1} of ${attestation.certChain!!.certificates.size}",
                    ...
                    IconButton(
                        enabled = (certNumber < attestation.certChain!!.certificates.size - 1),
                        onClick = {
                            certNumber += 1
                        }) {
                ...
                Column(
                    ...
                    Text(
                        text = styledX509CertificateText(
                            attestation.certChain!!.certificates[certNumber].javaX509Certificate.toString()),
...
private fun csaAttestation(
    ...
    return cloudSecureArea!!.getKeyInfo("testKey").attestation.certChain!!
}
