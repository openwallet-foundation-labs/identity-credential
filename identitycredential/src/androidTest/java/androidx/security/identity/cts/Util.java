...
class Util {
    ...
    private static byte[] signatureDerToCose(byte[] signature) {
        ...
        byte[] rBytes = stripLeadingZeroes(Arrays.copyOfRange(signature, rOffset + 2, rOffset + rSize + 2));

        int sOffset = rOffset + 2 + rSize;
        int sSize = signature[sOffset + 1];
        byte[] sBytes = stripLeadingZeroes(Arrays.copyOfRange(signature, sOffset + 2, sOffset + sSize + 2));
    ...
    public static boolean coseSign1CheckSignature(byte[] signatureCose1,
        ...
        byte[] encodedProtectedHeaders = ((co.nstant.in.cbor.model.ByteString) items.get(0)).getBytes();
    ...
    static int cborDecodeInt(@NonNull byte[] data) {
...
Certificate:
    ...
    static SecretKey calcEMacKeyForReader(PublicKey authenticationPublicKey, PrivateKey ephemeralReaderPrivateKey) {
        try {
    ...
    static @NonNull byte[] createItemsRequest(
        ...
        for (RequestNamespace requestNamespace : requestedEntryNamespaces) {
            MapBuilder<MapBuilder<MapBuilder<CborBuilder>>> entryNameMapBuilder =
                    nsMapBuilder.putMap(requestNamespace.getNamespaceName());
