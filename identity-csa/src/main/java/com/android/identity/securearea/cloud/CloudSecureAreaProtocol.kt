 ...
 * - Client Registration
 *   - This includes the client and the server exchanging keys (`DeviceBindingKeu` and
 ...
 *
 * The following flows are all implemented on top of E2EE.
 * - Create Key
...
object CloudSecureAreaProtocol {
     ...
     *
     * If successful, this concludes the registration process and the device stores
     * `serverState` locally as "registration context". See [E2EESetupRequest0] for
     * where it's used.
     */
    ...
    data class SetPassphraseRequest0(
        val passphrase: String,
    ) : Command()

    data class SetPassphraseResponse0(
        val serverState: ByteArray
    ) : Command()
