...
class CloudSecureAreaServer(
    ...
    data class RegisterState(
        ...
        var clientPassphraseSet: Boolean = false,
    ...
    private fun doRegisterRequest0(request0: RegisterRequest0,
        ...
        state.clientPassphraseSet = false
        state.clientPassphrase = ""
    ...
    private fun doSetPassphraseRequest0(
        request0: CloudSecureAreaProtocol.SetPassphraseRequest0,
        remoteHost: String,
        e2eeState: E2EEState
    ): Pair<Int, ByteArray> {
        // Protect against an attacker trying to change the passphrase
        if (e2eeState.context!!.clientPassphraseSet) {
            Logger.w(TAG, "$remoteHost: Passphrase already configured, cannot reconfigure")
            return Pair(403, "Passphrase cannot be reconfigured".toByteArray())
        }
        e2eeState.context!!.clientPassphraseSet = true
        e2eeState.context!!.clientPassphrase = request0.passphrase
        val response0 = CloudSecureAreaProtocol.SetPassphraseResponse0(
        ...
        Logger.d(TAG, "$remoteHost: doSetPassphraseRequest0: Passphrase configured")
        ...
        try {
            ...
            if (state.keyContext!!.passphraseRequired) {
                ...
                if (lockedOutDuration != null) {
                    Logger.i(
                        TAG, "$remoteHost: SignRequest1: Too many passphrase attempts, " +
                ...
                if (request1.passphrase != e2eeState.context!!.clientPassphrase) {
                    passphraseFailureEnforcer.recordFailedPassphraseAttempt(state.keyContext!!.clientId!!)
    ...
    private fun doKeyAgreementRequest1(
        ...
        try {
            ...
            if (state.keyContext!!.passphraseRequired) {
                ...
                if (lockedOutDuration != null) {
                    Logger.i(
                        TAG, "$remoteHost: KeyAgreementRequest1: Too many passphrase attempts, " +
                                "locked out for $lockedOutDuration"
                    )
    ...
    private fun handleCommandInternal(
        ...
        return when (command) {
            ...
            is CloudSecureAreaProtocol.SetPassphraseRequest0 -> doSetPassphraseRequest0(command, remoteHost, e2eeState!!)
