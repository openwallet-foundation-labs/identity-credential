...
class EngagementParserTest {
    ...
    fun testDeviceRequestEngagementWithVectors() {
        ...
        Assert.assertEquals(1, connectionMethods.size.toLong())
        Assert.assertTrue(connectionMethods[0] is ConnectionMethodBle)
        val cmBle = connectionMethods[0] as ConnectionMethodBle
        Assert.assertFalse(cmBle.supportsPeripheralServerMode)
        Assert.assertTrue(cmBle.supportsCentralClientMode)
        Assert.assertNull(cmBle.peripheralServerModeUuid)
        Assert.assertEquals(
