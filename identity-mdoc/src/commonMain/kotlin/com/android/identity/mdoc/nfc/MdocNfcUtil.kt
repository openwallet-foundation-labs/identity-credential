...
object MdocNfcUtil {
    ...
    private fun calculateHandoverSelectPayload(alternativeCarrierRecords: List<ByteArray>): ByteArray {
        ...
        for (acRecordPayload in alternativeCarrierRecords) {
            acRecords.add(NdefRecord(
                ...
                "ac".encodeToByteString(),
                ByteString(),
                ByteString(acRecordPayload)
            ))
        }
    ...
    private fun createNdefMessageHandoverSelectOrRequest(
                ...
                if (Logger.isDebugEnabled) {
                    Logger.d(
                        TAG, "ConnectionMethod $cm: alternativeCarrierRecord: "
                                ...
                                + "${records.first.payload.toHexString()}"
                    )
                }
        ...
        records.add(
            NdefRecord(
                NdefRecord.Tnf.WELL_KNOWN,
                (if (isHandoverSelect) "Hs" else "Hr").encodeToByteString(),
                ByteString(),
                ByteString(hsPayload)
            )
        )
        if (encodedDeviceEngagement != null) {
            records.add(
                NdefRecord(
                    ...
                    "iso.org:18013:deviceengagement".encodeToByteString(),
                    "mdoc".encodeToByteString(),
        ...
        if (encodedReaderEngagement != null) {
            records.add(
                NdefRecord(
                    NdefRecord.Tnf.EXTERNAL_TYPE,
                    "iso.org:18013:readerengagement".encodeToByteString(),
                    "mdocreader".encodeToByteString(),
                    ByteString(encodedReaderEngagement)
                )
            )
        }
    ...
    @OptIn(ExperimentalStdlibApi::class)
    fun parseHandoverSelectMessage(ndefMessage: NdefMessage): ParsedHandoverSelectMessage? {
        ...
        for (r in ndefMessage.records) {
            ...
            //
            Logger.i(TAG, "tnf=${r.tnf} type=${r.type.decodeToString()}")
            if (r.tnf == NdefRecord.Tnf.WELL_KNOWN && r.type.decodeToString() == "Hs") {
            ...
            if (r.tnf == NdefRecord.Tnf.MIME_MEDIA || r.tnf == NdefRecord.Tnf.EXTERNAL_TYPE) {
                val cm = connectionMethodFromNdefRecord(r, true)
                if (cm != null) {
        ...
        return ParsedHandoverSelectMessage(phsmEncodedDeviceEngagement.toByteArray(), phsmConnectionMethods)
    }

    fun connectionMethodFromNdefRecord(record: NdefRecord, isForHandoverSelect: Boolean): ConnectionMethod? {
        // BLE Carrier Configuration record
        //
        if (record.tnf == NdefRecord.Tnf.MIME_MEDIA &&
            record.type.decodeToString() == "application/vnd.bluetooth.le.oob" &&
        ...
        //
        if (record.tnf == NdefRecord.Tnf.MIME_MEDIA &&
            record.type.decodeToString() == "application/vnd.wfa.nan" &&
            record.id.decodeToString() == "W") {
            TODO()
            /*
        ...
        if (record.tnf == NdefRecord.Tnf.EXTERNAL_TYPE &&
            record.type.decodeToString() == "iso.org:18013:nfc" &&
    ...
    private fun bleToNdefRecord(
        ...
        val record = NdefRecord(
            ...
            "application/vnd.bluetooth.le.oob".encodeToByteString(),
