...
public class SimpleIssuerTrustStore implements IssuerTrustStore {
	...
	private Map<X500Principal, X509Certificate> trustedCertMap = new HashMap<>();
	...
	public SimpleIssuerTrustStore(KeyStore trustStore) {
		...
		try {
			Map<X500Principal, X509Certificate> trustedCertMap = new HashMap<>();
			...
			while (aliasEnum.hasMoreElements()) {
				...
				if (trustStore.isCertificateEntry(alias)) {
					...
					trustedCertMap.put(trustedCert.getSubjectX500Principal(), trustedCert);
				}
			}
...
//		}
	...
	public List<X509Certificate> createCertificationTrustPath(List<X509Certificate> chain) {
		...
		while (certIterator.hasNext()) {
			...
			X500Principal issuerX500Principal = currentCert.getIssuerX500Principal();
			trustedCert = trustedCertMap.get(issuerX500Principal);
			if (trustedCert != null) {
...
//			throw new CertificateException("Document Signer certificate is not a signing certificate");
		...
		while (certIterator.hasNext()) {
			...
			if (!prevCert.getIssuerX500Principal().equals(caCert.getSubjectX500Principal())) {
